...
- **拒绝平庸**：不要只生成简单的"盟友"或"敌对"关系。尝试构建"亦敌亦友"、"相爱相杀"、"表面盟友实则利用"等复杂关系。
- **情感纠葛**：挖掘角色之间深层的情感纽带（如：失散多年的亲人、被背叛的旧情人、恩师的私生子）。
- **意外连接**：让看似无关的两个角色之间存在意想不到的联系（例如：反派竟然是主角的资助人）。
- **动态变化**：在关系描述中暗示这种关系可能发生的变化（例如："目前是盟友，但随着利益分歧可能会决裂"）。
